...
## 5. Security findings
**Risk: HIGH (6.8/10).** Strong DB RLS, but the storage bucket is world-readable and the upload function is open. Plus prompt-injection exposure, wildcard CORS, client-side-only quotas/password checks, and a `SECURITY DEFINER` function without a pinned `search_path` (migration provided). Full analysis + OWASP mapping + remediation plan: [security-audit.md](audits/security-audit.md).
